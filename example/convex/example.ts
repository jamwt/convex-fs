...
import { exposeApi } from "@convex/fs";
...
export const addComment = mutation({
  ...
  handler: async (ctx, args) => {
    return await ctx.runMutation(components.fs.lib.add, {
      text: args.text,
...
export const listComments = query({
  ...
  handler: async (ctx, args) => {
    return await ctx.runQuery(components.fs.lib.list, {
      targetId: args.targetId,
    });
  },
});
...
export const translateComment = action({
  ...
  handler: async (ctx, args) => {
    return await ctx.runAction(components.fs.lib.translate, {
      baseUrl: BASE_URL,
...
// the component's API:
export const { list, add, translate } = exposeApi(components.fs, {
